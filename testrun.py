...
ua_h2o_list = [0.1*float(e) for e in ua_h2o_list] # mm-1
...
test_ri = [1, 1.5, 1.34, 1.4, 1.39, 1.4, 1.38, 1.44, 1]
test_sc = [0, 100, 45, 30, 35, 25, 30, 5]
test_ani = [0, 0.86, 0.8, 0.9, 0.95, 0.8, 0.95, 0.75]
test_thick = [0, 0.01, 0.08, 0.18, 0.28, 1.88, 2, 8] # 0, 0.01, 0.07, 0.1, 0.1, 1.6, 0.12, 6
...
def ua_me(wl):
    return 1.7 * (10**11) / (wl**(3.48)) 

# 15. S. Prahl, “Optical Absorption of Hemoglobin ” (1999)
...
def test_absorp():
    ...
    plt.title("Absorption coefficients for different chromophores")
...
def photon_moving(N, wl):
    ...
    #fig.write_image("resultaat_test_matched.pdf")
    #plt.scatter(x_list, y_list, c= W_surf)
    #plt.colorbar()
    #plt.title("Propagation of 5000 photons through skin at wavelength of 600 nm")
    #plt.xlim([-0.5, 0.5])
    #plt.ylim([-0.5, 0.5])
    #plt.xlabel('depth')
    #plt.scatter(r_list, z_list, c = W_list_sc)
    #plt.show()
    return R_d/N, T_d/N, A_d/N

#start = time.time()
#photon_moving(5000, 450)
#end = time.time()
#print(end - start)

def refl_spec(wl_start, wl_end):
    W_list_T = []
    W_list_R = []
    wl_list_it = []

    for wavelength in np.arange(wl_start, wl_end, 5):
        wl = wavelength
        R_d, T_d, A_d = photon_moving(5000, wl)
        W_list_R.append((R_d))
        W_list_T.append(T_d)
        wl_list_it.append(wl)
    return W_list_T, W_list_R, wl_list_it

start = time.time()
print("hello")
trans, refl, golfl = refl_spec(450, 700) 
end = time.time()
print(end - start)
