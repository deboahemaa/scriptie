...
def us_epidermis(wl, i):
    return 66.7*(0.29*(wl/500)**(-4) + 0.71*(wl/500)**(-0.689))

# function to calculate scattering coefficient for dermal layers
def us_dermis(wl, i):
    return 43.6*(0.41*(wl/500)**(-4) + 0.59*(wl/500)^(-0.562))

# function to calculate scattering coefficient for subcutaneous fat
def us_subc(wl, i):
    return 34.2*(0.26*(wl/500)**(-4) + 0.74*(wl/500)^(-0.567))
...
def image():
    ...
    np.savetxt("watercorr_txt", refl) # save reflectance per wl in range (wl_begin, wl_end, 5)
