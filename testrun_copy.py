...
ua_h2o_list = [float(e) for e in ua_h2o_list] # mm-1
...
test_ri = [1, 1.5, 1.34, 1.4, 1.39, 1.4, 1.38, 1.44, 1] # 1, 1.5, 1.34, 1.4, 1.39, 1.4, 1.38, 1.44, 1
test_ani = [0, 0.86, 0.8, 0.9, 0.95, 0.8, 0.95, 0.75] # 0, 0.86, 0.8, 0.9, 0.95, 0.8, 0.95, 0.75
test_thick = [0, 0.001, 0.008, 0.018, 0.028, 0.188, 0.20, 0.80] # 0, 0.01, 0.07, 0.1, 0.1, 1.6, 0.12, 6
...
def ua_other(wl):
    return 0.0244 + 8.53 / np.exp((wl-154)/66.2)

#absorption coefficient of melanin
def ua_me(wl):
    C_me, C_h2o, C_blood = conc(2)
    return ((2.37*10**4)*np.exp(-0.0056*wl) + (1.01*10**5)*np.exp(-0.0087*wl)) * C_me 
...
def test_refl():
    ...
    plt.yscale("log")
    plt.ylabel("scattering coefficient μ_a (mm E-1)")
    plt.xlabel("wavelength λ (nm)")
    plt.legend()
    plt.show()
    return
...
def test_abs_lay():
    ...
    dbnd = []
    sf = []
    layer = [strat, le, pd, ubnd, rd, dbnd, sf]
    name = ['strat', 'le', 'pd', 'ubnd', 'rd', 'dbnd', 'sf']
    for i in range(len(layer)):
        C_me, C_h2o, C_blood = conc(i+1)
        current_layer = layer[i]
        for wl in range(300,901):
            if i == 1:
                l.append(wl)
            current_layer.append(ua_layer(i+1, wl))
        layer[i] = np.copy(current_layer)
    for k in range(len(layer)):
        label1 = name[k]
        plt.plot(l,layer[k], label=label1)
    plt.xlabel('wavelength (nm)')
    plt.ylabel('Absorption coeffecient ua (/cm)')
    plt.yscale("log")
    plt.legend()
    plt.show()
    return
...
#            S = 0.4
        ...
        W_list_sc.append(W)
        r_squared = coord[0]**2 + coord[1]**2
        r_list.append(np.sqrt(r_squared))
    ...
    print(R_d/N, T_d/N)
    #print(verstrooi_phi)
    #plt.hist(verstrooi_phi)
    #plt.hist(verstrooi_theta)
    #plt.title('Histogram cosine of deflection angle theta')
    #plt.xlabel('cos(θ)')
...
def image():
    trans, refl, golf, spectral = refl_spec(380, 780, 2500) # wl_begin, wl_end, N_photons
    img = Image.new('RGB', (25, 25)) # 25 x 25 picture

    #np.savetxt("refl_spect_pixel_t0conc1", spectral) # save spectral images
    np.savetxt("REFL_L17", refl) # save reflectance per wl in range (wl_begin, wl_end, 5)
    np.savetxt("TRANS_L17", trans)
